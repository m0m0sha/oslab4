...
int main()
{
	...
	FILE* file1, * file2;
	string nameLogfile;
	nameLogfile = "C:\\Users\\vova_\\source\\repos\\oslab4\\WriterLogs\\writer_log_file_" + to_string(GetCurrentProcessId()) + ".txt";
	file1 = fopen(nameLogfile.c_str(), "w");
	string nameLogfileExcel;
	nameLogfileExcel = "C:\\Users\\vova_\\source\\repos\\oslab4\\WriterExcel\\writer_log_file_" + to_string(GetCurrentProcessId()) + ".txt";
	...
	while (GetTickCount() < start + 15000)
	{
		...
		fprintf(file2, "%d %d\n", GetTickCount(), 0);
		DWORD pageSemOne = WaitForMultipleObjects(PageNum, WrSem, FALSE, INFINITE);//Waiting for the semaphore page to get the value 1
		fprintf(file1, "Process: %d\t  State: WRITING page %d\t  Time: %d  \n", GetCurrentProcessId(), pageSemOne, GetTickCount());
		fprintf(file2, "%d %d\n", GetTickCount(), 1);
		...
		fprintf(logs, "%d %d\n", GetTickCount(), 1);
		ReleaseSemaphore(RdSem[pageSemOne], 1, NULL);
		fprintf(logs, "%d %d\n", GetTickCount(), 0);
		fclose(logs);
		fprintf(file1, "Process: %d\t  State: RELEASED  \t\t  Time: %d  \n\n", GetCurrentProcessId(), GetTickCount());
		fprintf(file2, "%d %d\n", GetTickCount(), 2);
	}
	return 0;
}
